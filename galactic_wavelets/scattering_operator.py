...
class ScatteringOp(torch.nn.Module):
    ...
    def __init__(
        ...
        self.mean_op = lambda x: x.nanmean(
            dim=(-3, -2, -1)
        )  # Average over the mesh grid (ignoring NaNs)
    ...
    def forward(
        ...
        wt_op, abs_op, moments_op, mean_op = (
            self.wt_op,
            self.abs_op,
            self.moments_op,
            self.mean_op,
        ...
        if awtx is not awtx_unmasked:
            del awtx  # To save memory
        ...
        if self.scattering:
            ...
            s2_x = torch.zeros(
                (M, Jeff * (Jeff - 1) // 2, L, L), dtype=s1_x.dtype, device=s1_x.device
            )
            cnt = 0
            for j1 in range(Jeff - 1):
                nb_scales = Jeff - 1 - j1
                for t1 in range(L):
                    s2_x[:, cnt : cnt + nb_scales, t1] = mean_op(
                        moments_op(abs_op(wt_op(awtx_unmasked[j1, t1], jmin=j1 + 1)))
                    )
                cnt += Jeff - 1 - j1
