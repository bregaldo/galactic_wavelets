...
def mask_erosion_torch(mask, kernel):
    ...
    kernel_support = torch.argwhere(kernel)  # List of nonzeros pixels in the kernel of shape (nb_nonzero_pixels, ndim)
    kernel_new_shape = []
    for i in range(kernel.ndim):
        M = kernel.shape[i]
    ...
    kernel_reshaped = kernel[tuple([slice(-K + M // 2, M // 2 + K + 1) for K, M in zip(kernel_new_shape, kernel.shape)])] # Make a window with odd number of pixels/dim centered on the support of the kernel
    mask_padded = F.pad(mask, sum(tuple([(K, K) for K in kernel_new_shape])[::-1], ()), mode='constant', value=True) # Pad with True

    kernel_time_reversal = torch.flip(kernel_reshaped, dims=spatial_dims) # F.conv operations compute the cross-correlation intestead of the convolution, so we need to time-reverse the kernel

    mask_conv = conv(mask_padded.unsqueeze(0).float(), kernel_time_reversal.unsqueeze(0).unsqueeze(0).float(), padding='valid')
