...
def lanusse_scaling_function_3d(r, kc=np.pi):
    ...
    mask = r != 0.0
    r_masked = r[mask]
    res[mask] = c / r_masked * (-768*(kc*r_masked*np.cos(kc*r_masked/4) \
                                      - 4*np.sin(kc*r_masked/4))*np.sin(kc*r_masked/4)**3/(kc**3*r_masked**5))
...
def lanusse_mother_wavelet_3d(r, kc=np.pi):
    ...
    Returns:
        array: Values on the grid.
    """
    return lanusse_scaling_function_3d(r, kc=2*kc) - lanusse_scaling_function_3d(r, kc=kc)

def lanusse_fwavelet_3d(kc: float, grid_or_shape, fourier=True, axes_weights=None, aliasing=True, return_grid=False):
    ...
    if kc > np.pi/2 and aliasing: # Lanusse wavelet function is zero beyond 2*kc
        ...
        grid = check_grid(tuple([extend_factor*i for i in shape]), axes_weights=axes_weights)
...
def angular_gaussians_bank(grid, orientations, angular_width=np.pi/4):
    ...
    for i, orientation in enumerate(orientations):
        ...
        if np.dot(orientation, z_axis) < 1 - 1e-5:
            ...
            rot3db = np.array([[np.cos(-angle), -np.sin(-angle), 0],
        ...
        elevation = np.arccos(np.clip(z / (radii + 1e-5), -1.0, 1.0)) # Somehow, in specific circumstances, z / (radii + 1e-5) can be outside [-1, 1]
...
def lanusse_fwavelet_3d_bank(grid_or_shape,
                             J=None, Q=1,
                             kc=np.pi,
                             orientations=None, angular_width=np.pi/4,
                             axes_weights=None,
                             aliasing=True,
                             use_mp=True):
    ...
    # Orientations check
    oriented = orientations is not None and angular_width is not None # Are we building oriented wavelets or not?
    if oriented:
    ...
    # Build  wavelets
    if use_mp:
        build_bp_para_loc = partial(lanusse_fwavelet_3d_bank_para, grid_or_shape=grid, orientations=orientations, angular_width=angular_width,
                                                    axes_weights=axes_weights, aliasing=aliasing, use_mp=False)
        ...
        for i, kc in enumerate(kcs):
            ...
            fw, g = lanusse_fwavelet_3d(kc, grid, axes_weights=axes_weights, aliasing=aliasing, return_grid=True)

            if oriented:
                # Axis weighting for Fourier space
                if axes_weights is not None:  # Need to compensate previous weighting in physical space
                    for j in range(len(shape)):
                        g[j] /= axes_weights[j]**2
                for j in range(len(shape)): # To get frequencies
                    g[j] /= shape[j]
                angular_gaussians = angular_gaussians_bank(g, orientations_rescaled, angular_width=angular_width)
            ...
            if aliasing and kc > np.pi/2:
                extend_factor = int(np.ceil(kc / (np.pi/2)))
                fw = fw.reshape(fw.shape[:-3] + (extend_factor, shape[0], extend_factor, shape[1], extend_factor, shape[2])).sum(axis=(-6, -4, -2))
...
def get_dodecahedron_vertices(half=False, los=None):
    ...
    if half:
        return vertices[np.dot(vertices, los if los is not None else np.array([0, 0, 1])) >= 0.0]
    else:
        return vertices
...
def get_icosphere2_vertices(half=False, los=None):
    ...
    Returns:
        array: Array of vertices.
    """
    vertices = np.array([   [ 0.        ,  0.52573111,  0.85065081],
